...
import {
  User, CreditCard, Archive, Calendar, Heart, Bell, HelpCircle,
  Menu, X, LogOut, Home, ChevronRight, QrCode
} from 'lucide-react'
...
export default function DashboardLayout({ children, profile }: { children: React.ReactNode; profile: Profile }) {
    ...
    <div className="flex flex-col h-full">
      <div className="p-6 border-b border-charcoal-800">
        <Link href="/" className="block">
          <p className="text-gold-500 font-serif text-lg font-bold tracking-widest">CIGAR CITY</p>
          <p className="text-gray-500 text-xs tracking-[0.3em]">LOUNGE</p>
        </Link>
      </div>
      ...
      <div className="p-4 border-t border-charcoal-800">
        <button
          ...
          className="flex items-center gap-3 px-4 py-3 rounded-lg text-base font-medium text-red-400 hover:bg-red-900/20 w-full transition-colors"
        >
          <LogOut size={20} />
          Sign Out
        </button>
  ...
  return (
    <div className="min-h-screen bg-charcoal-950 flex">
      <div className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 bg-charcoal-900 border-r border-charcoal-800">
        <Sidebar />
      </div>
      {sidebarOpen && (
