...
import { LayoutDashboard, Users, CreditCard, Archive, Coffee, Calendar, Image, Bell, HelpCircle, Settings, Shield, BarChart3, MessageSquare, Globe, Menu, X, LogOut } from 'lucide-react'
...
export default function AdminLayout({ children, profile }: { children: React.ReactNode; profile: Profile }) {
  ...
  const handleLogout = async () => { const s = createClient(); await s.auth.signOut(); window.location.href = '/' }

  const Sidebar = () => (
    <div className="flex flex-col h-full">
      <div className="p-5 border-b border-charcoal-800">
        <Link href="/"><p className="text-gold-500 font-serif text-base font-bold tracking-widest">CIGAR CITY LOUNGE</p><p className="text-red-400 text-xs tracking-widest mt-1">ADMIN PANEL</p></Link>
      </div>
      ...
      <div className="p-3 border-t border-charcoal-800">
        <div className="px-3 py-2 mb-2"><p className="text-gray-300 text-sm font-medium">{profile.first_name} {profile.last_name}</p><p className="text-gold-500 text-xs">{profile.role?.replace('_', ' ').toUpperCase()}</p></div>
        <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-red-400 hover:bg-red-900/20 w-full transition-colors"><LogOut size={18} />Sign Out</button>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-charcoal-950 flex">
      <div className="hidden lg:flex lg:w-60 lg:flex-col lg:fixed lg:inset-y-0 bg-charcoal-900 border-r border-charcoal-800"><Sidebar /></div>
      {sidebarOpen && (
